...
export default function CellModalButtons({ onClose, status, flagged, onResolve, onFlagged }: CellFooterButtonsProps) {
  return (
    <div
    ...
    >
      ...
      <button 
        ...
        onClick={onFlagged}  
      >
        {flagged ? 'Unflag panel': 'Flag panel'}
      </button>
      
      {status !== 'clean' && (
        <button 
          className="px-4 py-2 text-sm font-semibold rounded-[10px] text-white" style={{ background: '#5B3FD4' }}
          onClick={onResolve}  
        >
          Mark resolved
        </button>
